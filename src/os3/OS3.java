...
public class OS3 {
   ...
   public static void addWaiting2(Vector<Process> arr ,int time){
       boolean found = false;
           for(int i = 0 ; i < arr.size() ; i++){//loop in the array if the condition is true and it's not already in the waiting list add it
               if(time >= arr.elementAt(i).arrivalTime){//chech if apply
                   for(int j = 0 ; j < waitingList.size() ; j++){
                            if(arr.elementAt(i).name.equals(waitingList.elementAt(j).name) || arr.elementAt(i).name.equals(done.get(done.size() - 1).name)){//if the process is already in the waiting list
                                found = true;
                                break;
                            }
                        }
   ...
   public static void AgScheduling(Vector<Process>arr){
       ...
       while(burst_time > 0)
       {
           ...
           {
               if(done.get(done.size() - 1).Bursttime <= 0 || excute == done.get(done.size() - 1).quantem)
               {
                   burst_time -= excute;
                   if(done.get(done.size() - 1).Bursttime == 0 )
                   {
